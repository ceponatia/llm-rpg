import type { VADState, WeightedMemoryFusion } from '@rpg/types';

/**
 * Validation utilities for CAS data structures.
 * Converted from static class to object literal.
 */
export const validators = {
  /** Validate VAD emotional state values */
  isValidVADState(vad: VADState): boolean {
    return (
  ...
  isValidFusionWeights(weights: WeightedMemoryFusion): boolean {
  ...
  isValidImportanceScore(score: number): boolean {
  ...
  /** Sanitize text input */
  sanitizeText(text: string, maxLength = 10000): string {
    if (typeof text !== 'string') {
      return '';
    }
    // eslint-disable-next-line no-control-regex
    const controlCharsRegex = new RegExp('[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]', 'g');
    return text.trim().substring(0, maxLength).replace(controlCharsRegex, '');
  }
} as const;
