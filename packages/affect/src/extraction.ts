...
export function extractUserAffectSignals(input: ExtractionInput): AffectSignal {
  ...
  const complimentMatches: number = (text.match(/\b(thank you|thanks|great|awesome|love|nice|appreciate)\b/g) || []).length;
  const insultMatches: number = (text.match(/\b(stupid|dumb|hate|sucks|idiot)\b/g) || []).length;
  const empathyMatches: number = (text.match(/\b(sorry|feel for you|understand)\b/g) || []).length;
  const threatMatches: number = (text.match(/\b(kill|hurt|attack|destroy)\b/g) || []).length;
  ...
  const arousalCueScore: number = typeof input.arousalCueScore === 'number' ? clampUnit(input.arousalCueScore) : clampUnit((text.match(/!/g) || []).length / 5);
