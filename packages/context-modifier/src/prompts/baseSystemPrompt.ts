...
export function buildBaseSystemPrompt(vars: BasePromptVariables): string {
  const ragSection: string = vars.ragContext && vars.ragContext.length > 0
    ? `MEMORY SNIPPETS (relevant recent context)\n${vars.ragContext.map((s: string, i: number) => `- ${i + 1}. ${s}`).join('\n')}`
    : '';

  const emotionSection: string = vars.emotionalContext ? `CURRENT EMOTIONAL SNAPSHOT\n${vars.emotionalContext}` : '';
  const sceneSection: string = vars.sceneContext ? `SCENE CONTEXT\n${vars.sceneContext}` : '';
  const toolsSection: string = vars.toolsBlock ? `TOOLS\n${vars.toolsBlock}` : '';
